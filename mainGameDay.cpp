...
Scene* mainGameDay::createScene()
{
    return mainGameDay::create();
}
...
static void problemLoading(const char* filename)
{
    ...
    printf("Depending on how you compiled you might have to add 'Resources/' in front of filenames in mainGameDayScene.cpp\n");
}
...
void mainGameDay::createPlant(int type, int x, int y) {
	if (x > 5 || y > 9) return;
...
void mainGameDay::createZombie(int type, int x, int y = 1080) {
...
bool mainGameDay::init()
{
    ...
    auto pauseItem = MenuItemImage::create(
                                           "Scence/pause.png",
                                           "Scence/pause.png",
                                           CC_CALLBACK_1(mainGameDay::menuCloseCallback, this));
	...
	auto homeItem = MenuItemImage::create(
		"Scence/home.png",
		"Scence/home.png",
		CC_CALLBACK_1(mainGameDay::backHome, this));
	...
	auto musicItem = MenuItemImage::create(
		"Scence/sound.png",
		"Scence/sound.png",
		CC_CALLBACK_1(mainGameDay::pauseSound, this));
...
void mainGameDay::createBasicScene() {
	backGround = Sprite::create("Scence/Background1.png");
	backGround->setAnchorPoint(Vec2(0, 0));
	backGround->setPosition(-350, -32);
	...
	backGroundAudio->playBackgroundMusic("Music/Grasswalk.mp3", true);
	...
	schedule(CC_CALLBACK_1(mainGameDay::check, this), 0.07f, "check");
	schedule(CC_CALLBACK_1(mainGameDay::checkPlantisEaten, this), 0.06f, "checkeat");
	schedule(CC_CALLBACK_1(mainGameDay::createSun, this), 8.0f, "crtsun");
	schedule(CC_CALLBACK_1(mainGameDay::updateSun, this), 0.15f, "chksun");
	schedule(CC_CALLBACK_1(mainGameDay::updatePlantsAttack, this), 0.4f, "updateAttack");
	schedule(CC_CALLBACK_1(mainGameDay::plantEventListener, this), 0.05f, "plantListener");
	schedule(CC_CALLBACK_1(mainGameDay::updateCardWithSun, this), 0.05f, "updcrdcst");
	schedule(CC_CALLBACK_1(mainGameDay::Gen, this), 0.1f, "zbgen");
	schedule(CC_CALLBACK_1(mainGameDay::checkWin, this), 1.0f, "ckw");
...
void mainGameDay::checkWin(float dt) {
...
void mainGameDay::checkPlantisEaten(float dt) {
	for (auto &plant : plants) {
		...
		for (auto &zombie : zombies) {
			...
			if (zombie->getPositionX() - plant->getPositionX() <= 64 &&
				...
				/*if (!eattingEffect) {
					eattingEffect = true;
					scheduleOnce(CC_CALLBACK_1(mainGameDay::playEattingEffect, this), 0.8f, "etefct");
				}*/
				if (zombie->health <= 0) continue;
				...
				if (plant->getType() == 5) {
					...
					//log("qwq:%f %f\n", zombie->attackPoint * dt, zombie->attackPoint * dt * 2.0 / 10.0);
				plant->flash();
				if (plant->health <= 0) {
					zombie->setNormal();
...
void mainGameDay::check(float dt) {
	for(auto &plant: plants)
		for (auto &zombie : zombies) {
	...
	for (auto& zombie : zombies) {
		if (zombie->health > 0 && zombie->getPositionX() < -64.0 && !loseStatus) {
			...
			scheduleOnce(CC_CALLBACK_1(mainGameDay::gameLose, this), 3.0f, "lose");
		}
	}
}

void mainGameDay::createSun(float dt) {
	int posx = rand() % 1080;
	int posy = rand() % 600 + 32;
	auto Sun = sun::create();
	Sun->setPosition(Vec2(posx, 900 + rand() % 300));
	this->addChild(Sun, 3);
	auto moveTo = MoveTo::create(2000.0 / Sun->getDist(posx, posy), Vec2(posx, posy));
	Sun->runAction(moveTo);
}

void mainGameDay::updateSun(float dt) {
	sun* sunListener = sun::create();
	char tmp[11]; sprintf(tmp, "%d", sunListener->collect(1));
	sunLabel->setString(std::string(tmp));
}

void mainGameDay::updatePlantsAttack(float dt) {
	for (auto& plant : plants) {
...
void mainGameDay::updateCardWithSun(float dt) {
	sun* sunListener = sun::create();
...
void mainGameDay::plantEventListener(float dt) {
	sun* sunListener = sun::create();
...
void mainGameDay::gameLose(float dt) {
...
void mainGameDay::gameWin() {
	...
	backGroundAudio->pauseBackgroundMusic();
	Director::getInstance()->pause();
	scheduleOnce(CC_CALLBACK_1(mainGameDay::_backHome, this), 3.0f, "aftwin");
}

void mainGameDay::menuCloseCallback(Ref* pSender)
{
	...
	if (!pauseStatus) {
		...
		backGroundAudio->getInstance()->resumeBackgroundMusic();
		pauseStatus = false;
	}
...
void mainGameDay::backHome(Ref* pSender) {
	Director::getInstance()->popScene();
	backGroundAudio->getInstance()->pauseBackgroundMusic();
}

void mainGameDay::_backHome(float dt) {
	Director::getInstance()->resume();
	Director::getInstance()->popScene();
	backGroundAudio->getInstance()->pauseBackgroundMusic();
}
...
void mainGameDay::pauseSound(Ref* pSender) {
	static int status = 1;
	if (status) {
		backGroundAudio->getInstance()->pauseBackgroundMusic();
	}
